const { generateRandomString, generateEncryptedPassword } = require("../../../util");
var ejs = require("ejs");
...
module.exports = {
  ...
  fn: async function ({ data }, exits) {
    ...
    try {
      ...
      const user = await sails.models.user.create({ ...obj, is_form_submitted: true, status: 'approved' }).fetch();

      //TODO: send email/otp confirmation before login


      ejs.renderFile(path.join(__dirname, "..", "..", "..", "..", "views", "mails", "sendPassword.ejs"), { password, email: obj.email }, function (err, data) {
        if (err) {
      ...
      Time: ${moment().format()}`);
      return exits.success({ status: true, data: user, message: 'user created' })

    } catch (error) {
      sails.log.error(error)
      if (error.raw === 'Email already registered') {
        return exits.badRequest({ status: false, data: [], message: 'Email already registered' })//({ status: false, data: [], message: 'email already registered' })
      }
      return exits.badRequest({ status: false, data: [], message: 'unknown server error' })
    }
